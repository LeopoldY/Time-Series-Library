...
DEVICE_ID="${1:?Usage: bash run_device.sh DEVICE_ID [runner options]}"
shift
...
OUTPUT_ROOT="${OUTPUT_ROOT:-$PROJECT_ROOT/outputs/safe_two_stage_cleaned}"
cd "$PROJECT_ROOT"
for seq_len in 3 6 12 24; do
    "$PYTHON_BIN" -u run_safe_two_stage.py --device-id "$DEVICE_ID" --seq-len "$seq_len" \
